...
- [`docs/design/bulk-intake.md`](docs/design/bulk-intake.md) — how a merchant gets their catalog in and keeps it synced with the **least formatting work**. Only hard requirement = a stable ID per record. Intake ask: **separate tables + a 5-line data dictionary** (default), one flat file we split (fallback), mega-join **rejected**. Accept-fast **import session** lands raw verbatim, processes only on "that's everything". Both shapes → one internal model → **ASE interpret** (categories/axes/units, confidence) → **"I found this" confirm loop, never blocks**. Update basis = the SAME upsert-by-ID endpoint as initial load (replicates Algolia/Meilisearch): deltas via webhook or `modified_after`, **periodic full reconcile** as the self-healing **delete-catcher** (atomic index swap). Push-ingest API + import wizard already implement half of it. Read before scoping catalog intake / sync.
