...
- [`blog.md`](docs/policy/blog.md) — Multi-tenant blog. Single `post` table (RLS: anon can read `status='published'`); admin at `/content/posts`, public reading at `/blog/[slug]`. **v1 + v2 + v3 shipped**: Tiptap editor (HTML stored as text, sanitized via DOMPurify on read), markdown back-compat for v1 posts (`content_format` column), autosave, drafts/scheduled/published with Vercel cron `/api/v1/blog/publish-due` flipping scheduled→published every 5min, tags (`text[]` + GIN), TOC, reading time, `sitemap.xml`/`rss.xml`/`llms.txt`, host-based instance routing via `instance.domain` (per-request lookup, not middleware). **Backlog (directional, not committed):** Tiptap AI Toolkit (DIY on free Tiptap + Anthropic SDK), per-instance `brand_system`, image upload pipeline with brand-aware transforms (recolor / restyle / SVG-ify) — read the doc before scoping any AI-writing or AI-image work on the blog.
