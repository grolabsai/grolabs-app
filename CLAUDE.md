...
- `/dashboard/traffic` — GA4 traffic insights (dark "insights" design). Sections top→bottom: **Traffic** (Sessions / Avg session duration / Page views-per-session, then Users / new-vs-returning donut / Engagement), **Acquisition** (channels / geo / device-mix with device glyphs), **Pages** (landing / exit), then gray `Próximamente` shells at the bottom. Alerts UI was removed (no agreed thresholds — `ga4-integration.md` §15). All daily tiles show data **through yesterday** — the current day is excluded so a partial day never reads as a drop (`DATA_CUTOFF` in `ga4/fetchers.ts`); realtime active-users is the only live surface. A **Refresh data** button (and saving the Property ID) runs an on-demand pull. Full layout + visual conventions (border-less blocks, `--gl-panel` gray agent panel, the Tailwind-`ring` donut fix): `ga4-integration.md` §16; v1.1 corrections: §14.
...
- [`docs/design/wordpress-implementation-manual.md`](docs/design/wordpress-implementation-manual.md) — **assess → install → verify protocol for putting our plugins on a customer's WordPress.** Every installation is a different environment (cache layer, www/apex canonicalization, theme markup, neighboring plugins) and the failure mode is SILENT non-collection, so nothing is assumed: pre-install assessment checklist, verification probes (curl + the `test:e2e` preflight), a growing known-variations table (stale page cache serving plugin-less HTML, delete-reinstall wiping settings, the www/apex `grolabs_bid` identity split, drawer-toggle "close" buttons, the 5-min results transient), and the config-change purge protocol. Read before onboarding a WordPress merchant or debugging "plugin installed but no rows".
...
- [`docs/design/bulk-intake.md`](docs/design/bulk-intake.md) — how a merchant gets their catalog in, keeps it synced, **and receives enhanced products back** — least work on their side. Flow is **bidirectional**, so the canonical unit is the **product data object** (stable ID + nested variants/attrs/cats), built with the **generated SDK** (one `openapi.yaml` → both directions). Only hard requirement = a stable ID. Arrival/departure by source: **Shopify/WC = zero merchant code** (we pull + write via platform APIs); **custom = SDK push/pull**; **table dumps demoted** to a custom initial-load convenience we stitch into objects. Accept-fast **import session** lands raw → **ASE interpret** (cats/axes/units, confidence) → **"I found this" confirm loop, never blocks** → saves a **bidirectional mapping profile** (their fields ↔ ours, inverted for write-back). Update basis = the SAME upsert-by-ID endpoint as initial load (replicates Algolia/Meilisearch): deltas via webhook/`modified_after` + **periodic full reconcile** as the self-healing **delete-catcher** (atomic index swap). **Write-back = propose-then-approve, never silent.** Push-ingest API + `openapi.yaml` + import wizard already implement part of it. Read before scoping catalog intake / sync / enrichment write-back.
