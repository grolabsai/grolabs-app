...
- [ ] 🟠 **Finding 7 (cluster 4, MEDIUM) — patch vulnerable dependencies.** `npm audit` reports 9 prod vulns (6 high). 8 are auto-fixable via `npm audit fix` (non-major) — prioritize `dompurify` (the blog XSS defense depends on it) and `next`. `xlsx` (high, prototype pollution + ReDoS) has **no fix** and parses uploaded spreadsheets in the import path — replace (`exceljs`) or harden + document. [`audit/2026-08-02-auth-secrets-deps.md`](audit/2026-08-02-auth-secrets-deps.md). *Cluster-4 positives: `getUser()` (not `getSession`) everywhere; forced-password-change correctly gated on the JWT `amr` claim; no committed secrets; no secret/PII logging; OAuth `next` stays same-origin.*
- [ ] 🔎 **O-7 (cluster 4) — verify the SSO allowlist hook is live in prod.** The Before-User-Created hook that restricts SSO to pre-created emails lives in the Supabase dashboard, not the repo — can't be verified from code. App-side `NoAccess` backstop exists (no memberships → inert account). Confirm the hook is enabled, else unknown Google/MS users can create dead-end accounts.
