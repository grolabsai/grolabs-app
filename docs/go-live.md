...
- [ ] **Tenant/instance data isolation is PROVEN, not assumed.** This is the single most important MVP gate — RLS is what keeps client A from seeing client B's catalog, dashboards, and search data.
  - [ ] **Functional proof:** run `npm run test:isolation -- --yes` (against the target DB; needs the 3 Supabase env vars). It provisions two throwaway worlds, signs in as each user with the anon key, asserts cross-tenant reads return nothing + cross-tenant writes are rejected, then cleans up. Must report all checks passed. (`scripts/test-tenant-isolation.mjs`)
  ...
  - [ ] **Escalation path — currently UNTESTED and the highest-value gap.** Isolation hinges entirely on `instance_member.is_current` (that is literally what `current_instance_id()` returns). If a user can `UPDATE` their own `instance_member` row to point `is_current` at another tenant's `instance_id`, or `INSERT` a new one, every table's RLS opens at once. Add and pass an assertion for this before onboarding anyone.
...
- [x] **SEC-001 closed.** `isGroLabsAdmin()` is a real template-tenant membership check backed by `public.is_grolabs_admin()`. No edge stopgap needed.
  - [ ] Remaining sibling issue: funnel **shared-table writes still go through service-role with no app-level admin gate** (CLAUDE.md §17). Confirm no beta client can reach those server actions (`src/lib/actions/funnel.ts`), or gate them.
