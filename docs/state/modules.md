...
- **User & account management (planned, not built)** — [`docs/policy/user-management.md`](../policy/user-management.md) (Draft) specs, on top of the Identity/tenancy cluster: an admin-surface **"Clientes"** screen (GroLabs staff create a customer = tenant + `domain` + first instance + first Tenant Admin), an RRE **"Equipo"** screen (Tenant Admins create Admins/Members), **Google + Microsoft SSO** on the shared `/login` (GroLabs-styled, pre-created emails only), a `/cambiar-contrasena` forced first-login change, the **real `isGroLabsAdmin` gate** (closes SEC-001), and a GroLabs-staff cross-tenant switcher. New server actions: `createCustomerAccount`, `createTenantUser` (+ role/deactivate). Only schema change: `tenant.domain`. None of this is on disk yet.
