...
// Range vocabulary lives in ./range so client components can import without
// pulling in server-only deps. Re-exported for callers that already import
// it from this module.
import {
  DASHBOARD_RANGES,
  isDashboardRange,
...
export { DASHBOARD_RANGES, isDashboardRange, type DashboardRange };
...
export interface KpiSummaryTile {
  metric: "sessions" | "engagement_rate" | "users";
  current: number;            // latest finalized day value (or aggregate, if range > daily)
  baseline: number;           // 7-day rolling avg (excluding observed)
  deltaPct: number;           // signed % vs baseline
  status: "ok" | "firing";    // from ga4_alert
  spark: KpiSparkPoint[];     // last 14 daily values for the inline chart
  threshold: number;          // pp/percent threshold per policy (for the caption)
}

export interface UsersBreakdown extends KpiSummaryTile {
  newUsers: number;
...
export interface DashboardKpiSummary {
  sessions: KpiSummaryTile;
  engagement: KpiSummaryTile;
  users: UsersBreakdown;
}

/**
 * Bundle the three GA4 KPI tiles for the Resumen tab:
 *   sessions, engagement_rate, users (with new/returning breakdown).
 *
 * Reads ga4_session_daily for the last 14 days, derives the latest finalized
 * day vs 7-day baseline, and pulls firing-alert state. RLS scopes by instance.
 *
 * Range plumbing exists for future extension; for v1 the comparison baseline
 * is always 7-day rolling (matches the policy's locked thresholds and the
 * "vs promedio 7 días" copy on the mockup).
 */
export async function getDashboardKpiSummary(
  instanceId: number,
  _range: DashboardRange = "7d",
): Promise<DashboardKpiSummary | null> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("ga4_session_daily")
    .select(
      "date, sessions, users, new_users, returning_users, engagement_rate",
    )
    .eq("instance_id", instanceId)
    .order("date", { ascending: false })
    .limit(14);
  ...
  const observed = rows[rows.length - 1];
  const baselineRows = rows.slice(0, -1).slice(-7);
  ...
  const baseUsers =
    baselineRows.length > 0
      ? baselineRows.reduce((s, r) => s + r.users, 0) / baselineRows.length
      : 0;
  ...
  const { data: alerts } = await supabase
    ...
    .eq("instance_id", instanceId)
    .eq("status", "firing");
  const firing = new Set<AlertMetric>(
    ((alerts ?? []) as Array<{ metric: AlertMetric }>).map((a) => a.metric),
  );

  const sessionsCur = observed.sessions;
  const sessionsDelta =
    baseSessions > 0 ? ((sessionsCur - baseSessions) / baseSessions) * 100 : 0;
  const erCur = Number(observed.engagement_rate);
  const erDelta = (erCur - baseEr) * 100;
  const usersCur = observed.users;
  const usersDelta =
    baseUsers > 0 ? ((usersCur - baseUsers) / baseUsers) * 100 : 0;

  const sparkSessions: KpiSparkPoint[] = rows.map((r) => ({
    date: r.date,
    value: r.sessions,
  }));
  const sparkEr: KpiSparkPoint[] = rows.map((r) => ({
    date: r.date,
    value: Number(r.engagement_rate),
  }));
  const sparkUsers: KpiSparkPoint[] = rows.map((r) => ({
    date: r.date,
    value: r.users,
  }));

  const newUsers = observed.new_users;
  const returningUsers = observed.returning_users;
  const totalUsers = newUsers + returningUsers;
  const newPct = totalUsers > 0 ? newUsers / totalUsers : 0;
  const returningPct = totalUsers > 0 ? returningUsers / totalUsers : 0;
  ...
  return {
    sessions: {
      ...
      current: sessionsCur,
      baseline: baseSessions,
      deltaPct: sessionsDelta,
      status: firing.has("sessions") ? "firing" : "ok",
      spark: sparkSessions,
      threshold: SESSIONS_THRESHOLD_PCT * 100,
    },
    engagement: {
      metric: "engagement_rate",
      current: erCur,
      baseline: baseEr,
      deltaPct: erDelta,
      status: firing.has("engagement_rate") ? "firing" : "ok",
      spark: sparkEr,
      threshold: ENGAGEMENT_DROP_ABS * 100,
    },
    users: {
      metric: "users",
      current: usersCur,
...
// ── Aggregate traffic sources for the full-width stacked bar ─────────────────

export interface TrafficSourceSegment {
  channel: string;
  sessions: number;
  share: number; // 0..1
}
...
/**
 * Aggregate traffic sources by `default_channel_grouping` over a date window.
 * For "hoy"/"ayer" → the most-recent finalized day. For "7d"/"30d" → trailing
 * window. Used by the full-width stacked bar on the Resumen tab and on the
 * Traffic Detail page.
 */
export async function getTrafficSourcesAggregated(
  instanceId: number,
  range: DashboardRange = "7d",
): Promise<{ segments: TrafficSourceSegment[]; total: number }> {
  const supabase = await createClient();
  const { data: latest } = await supabase
    .from("ga4_traffic_daily")
    .select("date")
    ...
    .limit(1)
    .maybeSingle();
  if (!latest) return { segments: [], total: 0 };

  const observedDate = latest.date as string;
  const start = new Date(`${observedDate}T00:00:00Z`);
  let startDate = observedDate;
  if (range === "ayer") {
  ...
  const endDate = range === "ayer" ? startDate : observedDate;
  ...
  for (const r of rows) {
    agg.set(
      r.default_channel_grouping,
      (agg.get(r.default_channel_grouping) ?? 0) + r.sessions,
    );
  }
  const segments: TrafficSourceSegment[] = [...agg.entries()]
