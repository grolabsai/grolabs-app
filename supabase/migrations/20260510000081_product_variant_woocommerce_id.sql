...
--
-- Captured during Scout→WC push (writeback of the variation post id WC
...
-- (not Scout's variant_id).
--
-- Partial unique on (instance_id, woocommerce_id) — a variation post id is
-- unique inside a WooCommerce site (which is one Scout instance), and we
...
comment on column public.product_variant.woocommerce_id is
  'WC variation post ID. Captured during Scout→WC push or future variant restructuring of WC-imported wc_raw.variations. Used by the plugin to build add-to-cart URLs.';
