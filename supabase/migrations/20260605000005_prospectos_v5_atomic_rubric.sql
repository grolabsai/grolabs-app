...
--
-- APPLIED to live (project ixbbhwtpnebrhquunege) 2026-06-06 via Supabase MCP.
-- Adds the atomic-rubric layer designed in
...
-- Widen the legacy weight column (was numeric(4,3), max 9.999) so v5 integer
-- point-weights (per-check points that sum to their category's weight, up to ~30)
-- fit. Non-destructive: legacy fractional values ≤ 9.999 still fit. Aligns with
-- diagnostic_category.weight (also numeric(6,3)).
