-- Realtime open-cart summary for the Carts tab (RRE abandoned-cart recovery).
-- An "open cart" = a cart_id with an add/checkout event but NO completed order
-- and NO removal. Age = now − last cart event, bucketed by recovery urgency.
-- (Superseded by 20260627000008 (cap at 30d, drops d30plus) and 20260627000012
-- (recognizes the unified 'Added to cart' event) — kept for history.)
...
as $$
  ...
  select
    ...
    count(*) filter (where age_days >= 30)::bigint
  from open_carts o;
$$;
