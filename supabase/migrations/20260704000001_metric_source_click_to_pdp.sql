-- Add click_to_pdp (PDP views ÷ result clicks — the pdp-fence entry rate, per
-- conversion-measurement-foundations.md §"the fence table"). Daily-ratio shape,
-- same FULL JOIN pattern as pdp_to_cart. Everything else unchanged from _018.
...
SELECT COALESCE(a.instance_id, v.instance_id), COALESCE(a.day, v.day), 'pdp_to_cart'::text, 'event'::text,
  ...
  FROM (SELECT conv.instance_id, conv.day, count(*) FILTER (WHERE conv.event_name = 'Added to cart'::text) AS adds FROM conv GROUP BY conv.instance_id, conv.day) a
