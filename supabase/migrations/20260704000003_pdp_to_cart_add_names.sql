-- pdp_to_cart's numerator counted ONLY 'Added to cart', missing the legacy
-- 'Added to cart from PLP/PDP' names still present in history — inconsistent
-- with cart_to_checkout's denominator and recompute_cart(), which both use the
-- 3-name array. Found by the emulator round-trip (numerator 0 vs 20 adds).
-- The plugin (v0.14.0) sends the canonical 'Added to cart' + placement; the
-- array keeps legacy rows counted. Everything else unchanged from _001.
