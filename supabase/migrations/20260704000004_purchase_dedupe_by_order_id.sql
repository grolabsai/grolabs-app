-- Dedupe event-side purchases by order_id. The SDK/plugin may re-fire the
-- 'Completed order' EVENT on a retry/reload (the /orders ENTITY is idempotent,
-- events are not), which inflated checkout_to_purchase (2 purchases from 1
-- order in the SDK smoke) and search_to_purchase. Purchase counts over events
-- now use count(DISTINCT order_id) (+ raw count for legacy rows with NULL
-- order_id). session/user conversion use bool_or -> already idempotent.
-- Everything else unchanged from _003.
...
WITH cs AS (
  ...
  SELECT analytics_event.instance_id, analytics_event.created_at::date AS day, analytics_event.object_id
  FROM analytics_event WHERE analytics_event.event_type = 'view'::text
), so AS (
  SELECT instance_id, created_at::date AS day, amount, total_quantity
  FROM sales_order
), sc AS (
