...
--
-- v2 (2026-07-19, later the same day): adds a deterministic users population
-- (≈62% of sessions) and the revenue-efficiency KPIs revenue_per_session /
-- revenue_per_user (conversion-measurement foundations §14). All previous
-- keys reproduce byte-identical values. Applied via Supabase MCP; full-window
-- re-seed returned 1,274 rows (91 days × 14 keys).
...
BEGIN
  ...
  WITH days AS (
    ...
    SELECT *,
      ...
      GREATEST(5, round(sessions*(0.62+n7*0.04)))::numeric AS users,
  ...
  CROSS JOIN LATERAL (VALUES
    ...
    ('revenue_per_session', 'session', v4.sales, v4.sessions, round(v4.sales/NULLIF(v4.sessions,0), 4)),
    ('revenue_per_user', 'user', v4.sales, v4.users, round(v4.sales/NULLIF(v4.users,0), 4))
